...
void keyPressed()  // executed each time a key is pressed: sets the Boolean "keyPressed" till it is released   
    ...
    if(key=='2') showcentroidvec = !showcentroidvec;
    if(key=='3') tracetest = !tracetest;
    if(key=='4') colortri = !colortri;
    if(key=='5') singletrace = !singletrace;
    if(key=='6') tracesample = !tracesample;
    if(key=='7') showdiscpoints =!showdiscpoints;
    if(key=='8') createnewmesh=!createnewmesh;
    if(key=='9') showedge =!showedge;
    if(key=='0') resetBranching=true; 
    if(key=='-') refineCounter=max(0,refineCounter-1);
    if(key=='=') classifyvector=!classifyvector;

    if(key=='a') ;// used to insert one more arrow //{animate=!animate; if(!animate) ft=f;}
    if(key=='b') linear_spiral = !linear_spiral; 
    if(key=='c') cubic=!cubic; 
    if(key=='d') showconstrain = !showconstrain; 
...
String title ="Field-Aligned Triangle-Mesh",            name ="Rukai Zhao",
       subtitle = "",
