class ARROW 
  ...
  // CREATE
  ARROW() {}
  ARROW(pt Q, vec U) {P=P(Q); V=V(U);}
  // SHOW
  void show() {arrow(P,V);}
  }

// ARROW FUNCTIONS 
ARROW Arrow(pt Q, vec U) {return new ARROW(Q,U);}
void show(ARROW A) {A.show();}
ARROW LerpAverageOfArrows(ARROW A0, ARROW A1) {return Arrow(P(A0.P,A1.P),V(A0.V,A1.V));}
...
class ARROWRING 
  ...
  ARROWRING() {}
  void declare() {for (int i=0; i<maxnA; i++) A[i]=new ARROW(); }                           // creates all ARROWS, MUST BE DONE AT INITALIZATION
  void addArrow(pt Q, vec U) { A[nA].P=P(Q);  A[nA].V=V(U); nA++;  }                    // appends a point at position P
  void addArrow(ARROW B) { A[nA].P=P(B.P);  A[nA].V=V(B.V); nA++;  }                    // appends a point at position P
  void empty() {nA=0; }   // empties this object
  void showArrows() {for(int a=0; a<nA; a++) show(A[a]);}
  void showArrow(int a) {show(A[a]);}
  void showAverageArrows() 
    {
    for(int a=0; a<nA; a++) 
      show(AverageOfArrows(A[a],A[n(a)]));
    }
  int n(int a) {return (a+1)%nA;}  
  int p(int a) {return (a+nA-1)%nA;}  
