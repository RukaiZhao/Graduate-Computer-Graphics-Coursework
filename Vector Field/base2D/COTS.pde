...
class COTSMap 
  {
  int n=7;                                // cell-count in each direction
  pt A=P(), B=P(), C=P(), D=P();          // corners
  float mu=1, mv=1;                       // spiral scalings
  float au=0, av=0;                       // spiral angles
  float cx=0.5, cy=0.5, cr = 0.2; // control-circle center coordinates and radius wrt cell [0,0]
...
COTSMap() {}
  ...
  void showLabels() 
    {
    pen(black,1); showId(CM.A,"a"); showId(CM.B,"b"); showId(CM.C,"c"); showId(CM.D,"d"); 
 ...
 pt Inverse(pt M) 
  ...
  void showIsocurves()
    {
    noFill();
    int s=30; // sample count along each isocurve
    pen(red,2); 
    for(int i=0; i<=n; i++)
      {
      beginShape();
