...
boolean cw(pt A, pt B, pt C) {return det(V(A,B),V(B,C))>0; };                                        // triangle(A,B,C) is clockwise   
...
void label(pt P, String S) {text(S, P.x,P.y); }                                                 // writes string S next to P on the screen ( for example label(P[i],str(i));)
//void label(pt P, String S) {text(S, P.x-4,P.y+6.5); }                                                 // writes string S next to P on the screen ( for example label(P[i],str(i));)
void label(pt P, vec V, String S) {text(S, P.x+V.x,P.y+V.y); }                                  // writes string S at P+V
...
void showId(pt P, String S)                        // sows disk with S written inside
  {
  fill(white); 
  show(P,16); 
  fill(black); 
...
void v(float x, float y) {vertex(x,y);};                                                                      // vertex for drawing polygons between beginShape() and endShape()
void v(pt P, float u, float v) {vertex(P.x,P.y,u,v);};                                                // vertex for drawing polygons between beginShape() and endShape()
void v(float x, float y, float u, float v) {vertex(x,y,u,v);};                                                // vertex for drawing polygons between beginShape() and endShape()
...
void showIntLabel(pt P, int v)
      ...
      show(P,13); 
      fill(black); 
      if(v<10) label(P,str(v));  
      else label(P,V(3,0),str(v)); 
      noFill();
      }
...
pt L(pt A, float t, pt B) {return P(A.x+t*(B.x-A.x),A.y+t*(B.y-A.y));}
pt BiL(pt A, pt B, pt C, pt D, float u, float v) {return L(L(A,u,B),v,L(C,u,D));}

 // caplet, stumps...
 void caplet(pt A, float a, pt B, float b) //*01* displays Isosceles Trapezoid of axis(A,B) and half lengths rA and rB
  {
  vec I = U(A,B);
  float d = d(A,B), c=(a-b)/d, s=sqrt(1.-sq(c));
  pt PA = P(A,a*c,I), PB = P(B,b*c,I);
  float ra=a*s, rb=b*s;
  cone(PA,ra,PB,rb);
  }

 void stump(pt A, float a, pt B, float b) //*01* displays Isosceles Trapezoid of axis(A,B) and half lengths rA and rB
  {
  vec I = U(A,B);
  float d = d(A,B), c=(a-b)/d, s=sqrt(1.-sq(c));
  pt PA = P(A,a*c,I), PB = P(B,b*c,I);
  float ra=a*s, rb=b*s;
  halfCone(PA,ra,PB,rb);
  }

void halfCone(pt A, float rA, pt B, float rB) // displays Isosceles Trapezoid of axis(A,B) and half lengths rA and rB
  {
  vec T = U(A,B);
  vec N = R(T);
  pt LA = P(A,-rA,N);
  ...
  pt RA = P(A,rA,N);
  pt RB = P(B,rB,N);
  beginShape(); v(P(LB,LA)); v(P(RB,RA)); v(RA); v(LA); endShape(CLOSE);
  }
...
// create 
vec V() {return new vec(0,0); };    // make zero vector 
...
vec W(float u, vec U, float v, vec V, float w, vec W) {return W(W(S(u,U),S(v,V)),S(w,W));}                                   // uU+vV ( Linear combination)
...
// average 
vec V(vec U, vec V) {return new vec((U.x+V.x)/2,(U.y+V.y)/2);};                      // (1-s)U+sV (Linear interpolation between vectors)
