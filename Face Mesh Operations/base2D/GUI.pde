...
void keyPressed()  // executed each time a key is pressed: sets the Boolean "keyPressed" till it is released   
    ...
    if(key=='a') ; 
    if(key=='b') show_face = !show_face; 
    if(key=='c') P.resetOnCircle(P.nv);
    if(key=='d') ; 
    if(key=='e') show_red = !show_red;
    if(key=='f') add_arrow =!add_arrow;
    if(key=='g') ; 
    if(key=='h') ;
    if(key=='i') ; 
    if(key=='j') ;
    ...
    if(key=='l') ;
    if(key=='m') ;
    if(key=='n') next = face_mesh.N[next]; //iswing = false; isnext = !isnext;
    if(key=='o') ;
    if(key=='p') ;
    if(key=='q') ; 
    if(key=='r') ; // used in mouseDrag to rotate the control points 
    if(key=='s') {
    if(face_mesh.S[next]==-1)
      next = next;
    else if(face_mesh.S[next]!=-1)
      next = face_mesh.S[next];
}
...
String title ="Project 2 Face Graph",            name ="Rukai Zhao",
       subtitle = "   for Jarek Rossignac's CS6491 class",
